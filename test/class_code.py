...
class AgrosClass:
    ...
    def download_and_read_file(self):
        ...
        file_name_world = "ne_10m_admin_0_countries.zip"
        file_path = os.path.join(self.directory, file_name)
        file_path_world = os.path.join(self.directory, file_name_world)
        url_world = "https://www.naturalearthdata.com/http//www.naturalearthdata.com/download/10m/cultural/ne_10m_admin_0_countries.zip"
        flag_normal = False
        flag_world = False
        if os.path.exists(file_path):
            print(f"{file_name} already exists in {self.directory}")
            flag_normal = True
        if os.path.exists(file_path_world):
            print(f"{file_path_world} already exists in {self.directory}")
            flag_world = True
        if not flag_world:
            urllib.request.urlretrieve(url_world, file_path_world)
        else:
        ...
        zip = "https://www.naturalearthdata.com/http//www.naturalearthdata.com/download/10m/cultural/ne_10m_admin_0_countries.zip!ne_10m_admin_0_countries.shp"
        file_name = "downloads/ne_10m_admin_0_countries"
        file_path = os.path.join(self.directory, file_name)
        ...
        if os.path.exists(file_path):
           ...
           zip = "https://www.naturalearthdata.com/http//www.naturalearthdata.com/download/10m/cultural/ne_10m_admin_0_countries.zip"
    ...
    def download_csv_and_zip(self, csv_url, zip_url):
        req = requests.get(zip_url)
        filename = zip_url.split('/')[-1]
        # Create a subfolder for downloads if it doesn't already exist
        if not os.path.exists("downloads"):
            os.makedirs("downloads")
        
        # Check if the CSV and ZIP files already exist in the downloads folder
        csv_path = os.path.join("downloads", "data.csv")
        zip_path = os.path.join("downloads", filename)
        if os.path.exists(csv_path) and os.path.exists(zip_path):
            print("CSV and ZIP files already exist in downloads folder.")
        else:
            ...
            with open("downloads/"+filename,'wb') as output_file:
                output_file.write(req.content)
            print("ZIP file downloaded.")
        
        # Read the CSV file into a pandas DataFrame
        agros_df = pd.read_csv(csv_path)
        print(zip_path+"!ne_10m_admin_0_countries.shp")
        print(filename)
        # Extract the shapefile from the ZIP file and read it into a geopandas DataFrame
        doof = "download/"+filename+"!ne_10m_admin_0_countries.shp"
        zipfile = "zip:////Users/mundt/Documents/01_Nova_SBE/T3/Advanced_Programming_for_DataScience/Group_08/downloads"+filename+"!data/gadm36_AFG_1.shp"
        gdf = gpd.read_file(zipfile)
        # Return both DataFrames
        return agros_df, gdf

csv_url="https://raw.githubusercontent.com/owid/owid-datasets/master/datasets/Agricultural%20total%20factor%20productivity%20(USDA)/Agricultural%20total%20factor%20productivity%20(USDA).csv"
zip_url ="https://www.naturalearthdata.com/http//www.naturalearthdata.com/download/10m/cultural/ne_10m_admin_0_countries.zip"
agros = AgrosClass()
df, gdf = agros.download_csv_and_zip(csv_url, zip_url)
